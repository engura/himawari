  ...
  class Base
    ...
    def update_backgrnd
      ...
      Dir["#{data_path}/h_*.png"].sort { |x, y| y <=> x }.each do |pic|
        pic_time = pic[-8, 4]
        if sexy_pics.positive? && pic_time < LOCAL_MIDNIGHT && LOCAL_MIDNIGHT <= prev_pic
          complete = true
          break
        end
        sexy_pics.unshift(pic) if sexy_pics.positive? || pic_time < LOCAL_MIDNIGHT && LOCAL_MIDNIGHT <= prev_pic
        prev_pic = pic_time
      end
      # puts sexy_pics
